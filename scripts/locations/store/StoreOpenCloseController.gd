...
func request_toggle_store_open() -> void:
	...
	if not store._store_opened_today and not is_day_setup_complete():
		store._show_notification("Set up the shelf and stock items before opening.", 1.5)
...
func is_day_setup_complete() -> bool:
	return store._human_shelf_installed and store._human_items_placed >= NORMAL_STOCK_REQUIRED
...
func open_store() -> void:
	...
	store._show_task_complete_notice("store_opened", "Open sign flipped.")
...
func update_store_status_board(animated: bool = true) -> void:
	...
	if store.open_close_board != null and store.open_close_board.has_method("set_open_state"):
		store.open_close_board.call("set_open_state", store._store_open, animated)
...
func get_open_close_board() -> Node:
	...
	var group_board := store.get_tree().get_first_node_in_group("open_close_board")
