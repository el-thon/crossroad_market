...
func _enter_storage() -> void:
	...
	player.reparent(_current_storage, true)
	player.visible = true
	...
	player.global_position = spawn_marker.global_position if spawn_marker != null else Vector2(42, 68)
...
func _on_storage_return(_door_type: String) -> void:
	...
	if player != null:
		...
		player.set_process_unhandled_input(true)
		player.z_index = 0
		player.global_position = _get_storage_return_position()
	else:
...
func _enter_yard() -> void:
	...
	_set_store_world_active(false)
	player.reparent(_current_yard, true)
	player.visible = true
	player.set_process(true)
	player.set_physics_process(true)
	player.set_process_input(true)
	...
	player.global_position = spawn_marker.global_position if spawn_marker != null else Vector2(240, 136)
...
func _on_yard_return(_door_type: String) -> void:
	...
	if player != null:
		...
		player.set_process_unhandled_input(true)
		player.z_index = 0
		player.global_position = _get_yard_return_position()
	else:
...
func _fade_to_black() -> void:
	...
	tween.tween_property(_fade_rect, "modulate:a", 1.0, 0.35)
...
func _fade_from_black() -> void:
	...
	var tween := create_tween()
	tween.tween_property(_fade_rect, "modulate:a", 0.0, 0.35)
	await tween.finished
...
func _on_npc_spawn_requested(npc_data: NPCData) -> void:
	...
	if not npc.purchase_completed.is_connected(_on_npc_purchase):
		npc.purchase_completed.connect(_on_npc_purchase)

	if not npc.npc_exited.is_connected(_on_npc_exited):
